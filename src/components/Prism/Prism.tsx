...
const Prism = ({
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      ro.disconnect();
      if (animationType === 'hover') {
        if (onPointerMove) window.removeEventListener('pointermove', onPointerMove as unknown as EventListener);
        window.removeEventListener('mouseleave', onLeave);
        window.removeEventListener('blur', onBlur);
      }
      if (suspendWhenOffscreen) {
        const c = container as HTMLDivElement & { __prismIO?: IntersectionObserver };
        const io = c.__prismIO;
        if (io) io.disconnect();
        delete c.__prismIO;
      }
